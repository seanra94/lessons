...
- **In refit UI integrations, sync the refit panel before fetching the ship display or ship.** Methods such as `syncWithCurrentVariant` can rebuild or replace the displayed `ShipAPI` / ship-display object. If a mod gets the ship display first, then syncs, it may keep editing a stale ship with pre-refit weapon groups, even after the vanilla refit UI has accepted weapon reassignment and compacted empty weapon groups.
